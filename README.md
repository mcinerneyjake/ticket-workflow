...
The `gitignore` audit check requires `.claude/worktrees` and `node_modules` to be ignored —
**without a trailing slash**, which matches directories only and so misses a symlink: a worktree
materialised as one, or a worktree's `node_modules` linked to the primary's. An unignored worktree
shows up as a mountain of untracked files in the main checkout, which is exactly when someone
reaches for `git add -A` and commits another session's in-flight work. The check asks git for the
